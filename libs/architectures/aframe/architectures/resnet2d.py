...
class GroupNorm(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        print(f"sq_mean shape: {sq_mean.shape}")
        ...
        if self.num_groups != self.num_channels:
            ...
            print(f"stacked mean shape: {mean.shape}")
            mean = mean.reshape(
                -1, 2, self.num_groups, self.channels_per_group
            )
            print(f"reshaped mean shape: {mean.shape}")
            mean = mean.mean(-1, keepdims=True)
            print(f"mean mean shape: {mean.shape}")
            mean = mean.expand(-1, -1, -1, self.channels_per_group)
            print(f"expanded mean shape: {mean.shape}")
            mean = mean.reshape(-1, 2, self.num_channels, 1)
            print(f"reshaped expanded mean shape: {mean.shape}")
            mean, sq_mean = mean[:, 0], mean[:, 1]
            print(f"new mean shape: {mean.shape}")
            print(f"new sq_mean shape: {sq_mean.shape}")
...
def get_norm_layer(groups: Optional[int] = None) -> nn.Module:
    ...
    class NormLayer(torch.nn.GroupNorm):
